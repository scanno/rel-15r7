...
static void tegra_enable_i2c_slave(struct nvec_chip* nvec)
{
	...
	struct clk* i2c_fast_clk = nvec->i2c_fast_clk;

	/* If already enabled, avoid reenabling it */
	if (nvec->i2c_enabled)
		return;
	
	clk_enable(i2c_fast_clk);
	clk_enable(i2c_clk);
...
static int __devinit tegra_nvec_probe(struct platform_device *pdev)
{
	...
	i2c_fast_clk = clk_get_sys(pdata->clock, "i2c-fast");
	if (IS_ERR_OR_NULL(i2c_fast_clk)) {
		dev_err(nvec->dev, "failed to get clock tegra-i2c.2\n");
		goto failed;
	}
	
	nvec->i2c_fast_clk = i2c_fast_clk;
...
#ifdef CONFIG_PM
static int tegra_nvec_suspend(struct device *dev)
{
...
static int tegra_nvec_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct nvec_chip *nvec = platform_get_drvdata(pdev);
...
#ifdef CONFIG_PM
static const struct dev_pm_ops nvec_pm_ops = {
