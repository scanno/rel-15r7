...
#define LULZACTIVEQ_VERSION	(2)
#define LULZACTIVEQ_AUTHOR	"tegrak"

// if you changed some codes for optimization, just write your name here.
#define LULZACTIVEQ_TUNER "gokhanmoral-robertobsc-ejtagle"
...
struct cpufreq_lulzactiveq_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_lulzactiveq_cpuinfo, cpuinfo);
 ...
 */
#define DEFAULT_PUMP_DOWN_STEP 3
...
#if !defined(CONFIG_CPU_EXYNOS4210) && !defined(CONFIG_ARCH_TEGRA_2x_SOC)
...
static int cpufreq_governor_lulzactiveq(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_LULZACTIVEQ
static
#endif
struct cpufreq_governor cpufreq_gov_lulzactiveq = {
    .name = "lulzactiveq",
	.governor = cpufreq_governor_lulzactiveq,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

struct workqueue_struct *dvfs_lulzq_workqueue;
...
static unsigned int get_lulzfreq_table_size(struct cpufreq_lulzactiveq_cpuinfo *pcpu) {
...
static inline void fix_screen_off_min_step(struct cpufreq_lulzactiveq_cpuinfo *pcpu) {
...
static inline unsigned int adjust_screen_off_freq(
	struct cpufreq_lulzactiveq_cpuinfo *pcpu, unsigned int freq) {
...
static void cpufreq_lulzactiveq_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_lulzactiveq_cpuinfo *pcpu =
	...
	/*
	 * START lulzactiveq algorithm section
	 */
...
static void cpufreq_lulzactiveq_idle_start(void)
{
	struct cpufreq_lulzactiveq_cpuinfo *pcpu =
...
static void cpufreq_lulzactiveq_idle_end(void)
{
	struct cpufreq_lulzactiveq_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
...
static int cpufreq_lulzactiveq_up_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_lulzactiveq_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
...
static void cpufreq_lulzactiveq_freq_down(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
...
static void apply_hotplug_lock(void)
{
    ...
    struct cpufreq_lulzactiveq_cpuinfo *dbs_info;
	...
	queue_work_on(dbs_info->cpu, dvfs_lulzq_workqueue, work);
}
...
void cpufreq_lulzactiveq_min_cpu_lock(unsigned int num_core)
{
	...
	if (flag <= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_lulzq_workqueue, &dbs_info->up_work);
}
...
void cpufreq_lulzactiveq_min_cpu_unlock(void)
{
	...
	if (flag >= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_lulzq_workqueue, &dbs_info->down_work);
}
...
static struct attribute *lulzactiveq_attributes[] = {
...
static struct attribute_group lulzactiveq_attr_group = {
	.attrs = lulzactiveq_attributes,
    .name = "lulzactiveq",
};
...
static void dbs_check_cpu(struct cpufreq_lulzactiveq_cpuinfo *this_dbs_info)
{
	struct cpufreq_policy *policy;
	int num_hist = hotplug_lulzq_history->num_hist;
	...
	if (check_up()) {
		queue_work_on(this_dbs_info->cpu, dvfs_lulzq_workqueue,
...
{
    struct cpufreq_lulzactiveq_cpuinfo *dbs_info =
            container_of(work, struct cpufreq_lulzactiveq_cpuinfo, work.work);
	...
	queue_delayed_work_on(cpu, dvfs_lulzq_workqueue, &dbs_info->work, delay);
	mutex_unlock(&dbs_info->timer_mutex);
}
static inline void hotplug_timer_init(struct cpufreq_lulzactiveq_cpuinfo *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, dvfs_lulzq_workqueue,
...
static int cpufreq_lulzactiveq_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_lulzactiveq_idle_start();
		break;
	case IDLE_END:
		cpufreq_lulzactiveq_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_lulzactiveq_idle_nb = {
	.notifier_call = cpufreq_lulzactiveq_idle_notifier,
};

static void lulzactiveq_early_suspend(struct early_suspend *handler) {
	early_suspended = 1;
}

static void lulzactiveq_late_resume(struct early_suspend *handler) {
	early_suspended = 0;
}

static struct early_suspend lulzactiveq_power_suspend = {
	.suspend = lulzactiveq_early_suspend,
	.resume = lulzactiveq_late_resume,
...
void start_lulzactiveq(void)
{
	...
	up_task = kthread_create(cpufreq_lulzactiveq_up_task, NULL,
                 "klulzqup");

	sched_setscheduler_nocheck(up_task, SCHED_FIFO, &param);
	get_task_struct(up_task);

	idle_notifier_register(&cpufreq_lulzactiveq_idle_nb);
	register_early_suspend(&lulzactiveq_power_suspend);
}
...
void stop_lulzactiveq(void)
{
	...
	idle_notifier_unregister(&cpufreq_lulzactiveq_idle_nb);
	unregister_early_suspend(&lulzactiveq_power_suspend);
	pump_up_step = DEFAULT_PUMP_UP_STEP;
	pump_down_step = DEFAULT_PUMP_DOWN_STEP;
}

static int __init cpufreq_lulzactiveq_init(void)
{
	...
	if (!hotplug_lulzq_history) {
		pr_err("%s cannot create lulzactiveq hotplug history array\n", __func__);
		ret = -ENOMEM;
		goto err_queue;
	}

	dvfs_lulzq_workqueue = create_workqueue("klulzactiveq");
	if (!dvfs_lulzq_workqueue) {
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_lulzactiveq_timer;
	...
	INIT_WORK(&freq_scale_down_work,
		  cpufreq_lulzactiveq_freq_down);

	spin_lock_init(&up_cpumask_lock);
	spin_lock_init(&down_cpumask_lock);
	mutex_init(&set_speed_lock);

	return cpufreq_register_governor(&cpufreq_gov_lulzactiveq);
...
fs_initcall(cpufreq_lulzactiveq_init);
#else
module_init(cpufreq_lulzactiveq_init);
#endif

static void __exit cpufreq_lulzactiveq_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_lulzactiveq);
	kthread_stop(up_task);
	put_task_struct(up_task);
	destroy_workqueue(dvfs_lulzq_workqueue);
	destroy_workqueue(down_wq);
	kfree(hotplug_lulzq_history);
	kfree(rq_data);
}

module_exit(cpufreq_lulzactiveq_exit);
